...
def round_scores(student_scores):
    ...
    rounded_score_list= [round(scores) for scores in student_scores]
...
def count_failed_students(student_scores):
    ...
    failed_student=[scores for scores in student_scores if scores<=40]
...
def above_threshold(student_scores, threshold):
    ...
    above_threshold_scores= [score for score in student_scores if score >= threshold]
...
def letter_grades(highest):
    ...
    lower_threshold= [lower_limit for lower_limit in range(41,(highest),interval)]
...
def student_ranking(student_scores, student_names):
    ...
    students= [(f"{index+1}. {names}: {student_scores[index]}") 
