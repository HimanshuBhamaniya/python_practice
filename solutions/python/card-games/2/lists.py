...
def approx_average_is_average(hand: list):
    ...
    way_1 = (hand[0]+ hand[-1])/2
    index = len(hand)//2
    way_2 = hand[index]
    if way_1 == card_average(hand) or way_2 == card_average(hand):
        return True
    return False
...
def average_even_is_average_odd(hand: list):
    ...
    even_index_cards_avg = sum(hand[::2])/ len(hand[::2])
    odd_index_cards_avg = sum(hand[1::2])/ len(hand[1::2])
    return even_index_cards_avg == odd_index_cards_avg
