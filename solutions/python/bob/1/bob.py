...
def response(hey_bob):
    ...
    if hey_bob_c[len(hey_bob_c)-1] is '?' and not hey_bob_c.isupper():
        return 'Sure.'
    if hey_bob_c.isupper() and hey_bob_c[len(hey_bob_c)-1] is not '?':
        return 'Whoa, chill out!'
    if hey_bob_c[len(hey_bob_c)-1] is '?' and hey_bob_c.isupper():
