...
def remove_suffix_ness(word):
    ...
    if 'iness' in word:
        split_at_i = word.split('i')
        return split_at_i[0]+'y'
    return word[:-4]
...
def adjective_to_verb(sentence, index):
    ...
    if '.' in split_sentence[index]:
        remove_dot = split_sentence[index]
        return remove_dot[:-1]+'en'
